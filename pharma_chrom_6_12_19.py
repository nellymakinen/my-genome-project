...
def pharmacogenomics_analysis(input_dir, output_dir):
    """
    Perform pharmacogenomic analysis on SNP files from chromosomes 6, 12, and 19.
    ...
    # Chromosomes to filter (6, 12, and 19)
    chromosomes_to_analyze = [1, 6]
    ...
    for file_index, annotated_file in enumerate(annotated_files, start=1):
        ...
        # Filter SNPs to only include chromosomes 6, 12, and 19
        snps_filtered = snps[snps['Chromosome'].isin(chromosomes_to_analyze)]
        print(f"Found {len(snps_filtered)} SNPs from chromosomes 6, 12, and 19 in {annotated_file}.")
        
        results = []
        
        # Use tqdm for progress tracking in SNP processing
        for index, rsid in tqdm(enumerate(snps_filtered['Identifier'], start=1), total=len(snps_filtered), desc=f"Processing SNPs in {annotated_file}"):
